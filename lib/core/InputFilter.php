...
class InputFilter {
    ...
    /** 
      * Secure userinput from tags
      * @access public
      * @param Mixed $source - input string/array-of-string to be 'cleaned'
      * @return String $source - 'cleaned' version of input parameter
      * @author stamp
      */
    public function htmlencode($source) {
        // clean all elements in this array
        if (is_array($source)) {
            foreach($source as $key => $value)
                // replace &,<,> whith their html code
                if (is_string($value)) $source[$key] = htmlspecialchars($value);
            return $source;
        // clean this string
        } else if (is_string($source)) {
            // replace &,<,> whith their html code
